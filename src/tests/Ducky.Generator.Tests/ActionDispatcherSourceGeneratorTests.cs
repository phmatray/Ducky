...
public class ActionDispatcherSourceGeneratorTests
{
    ...
    private const string UnrelatedAttributeSource =
        ...
        namespace Demo.BlazorWasm.AppStore
        {
            ...
            public class DuckyActionLoggerAttribute : System.Attribute { }
        }

        """;

    private const string RecordWithUnrelatedAttributeSource =
        """

        namespace Demo.BlazorWasm.AppStore;

        [DuckyActionLogger]
        public record FakeAction(int Value);
    ...
    [Fact]
    public void DoesNotGenerateForUnrelatedAttributeWithDuckyActionInName()
    {
        // Arrange
        ActionDispatcherSourceGenerator generator = new();

        string[] sources = [DispatcherSource, UnrelatedAttributeSource, RecordWithUnrelatedAttributeSource];
        ...
        // Assert — only the DuckyActionAttribute.g.cs should be generated, no ActionDispatcher files
        runResult.GeneratedTrees
            .Where(tree => tree.FilePath.Contains("ActionDispatcher.FakeAction"))
            .ShouldBeEmpty();
    }
}
