// Copyright (c) 2020-2024 Atypical Consulting SRL. All rights reserved.
// Atypical Consulting SRL licenses this file to you under the GPL-3.0-or-later license.
// See the LICENSE file in the project root for full license information.

namespace Demo.Website1.Helpers;

// Palette from MudBlazor
// https://mudblazor.com/features/colors#material-colors-list-of-material-colors

/// <summary>
/// Defines a set of colors from the Material Design palette.
/// </summary>
public static class Palette
{
    /// <summary>
    /// The color black.
    /// </summary>
    public const string Black = "rgba(39,44,52,1)";

    /// <summary>
    /// The color white.
    /// </summary>
    public const string White = "rgba(255,255,255,1)";

    /// <summary>
    /// The color red.
    /// </summary>
    public const string Red = "#F44336";

    /// <summary>
    /// The color pink.
    /// </summary>
    public const string Pink = "#E91E63";

    /// <summary>
    /// The color purple.
    /// </summary>
    public const string Purple = "#9C27B0";

    /// <summary>
    /// The color deep purple.
    /// </summary>
    public const string DeepPurple = "#673AB7";

    /// <summary>
    /// The color indigo.
    /// </summary>
    public const string Indigo = "#3F51B5";

    /// <summary>
    /// The color blue.
    /// </summary>
    public const string Blue = "#2196F3";

    /// <summary>
    /// The color light blue.
    /// </summary>
    public const string LightBlue = "#03A9F4";

    /// <summary>
    /// The color cyan.
    /// </summary>
    public const string Cyan = "#00BCD4";

    /// <summary>
    /// The color teal.
    /// </summary>
    public const string Teal = "#009688";

    /// <summary>
    /// The color green.
    /// </summary>
    public const string Green = "#4CAF50";

    /// <summary>
    /// The color light green.
    /// </summary>
    public const string LightGreen = "#8BC34A";

    /// <summary>
    /// The color lime.
    /// </summary>
    public const string Lime = "#CDDC39";

    /// <summary>
    /// The color yellow.
    /// </summary>
    public const string Yellow = "#FFEB3B";

    /// <summary>
    /// The color amber.
    /// </summary>
    public const string Amber = "#FFC107";

    /// <summary>
    /// The color orange.
    /// </summary>
    public const string Orange = "#FF9800";

    /// <summary>
    /// The color deep orange.
    /// </summary>
    public const string DeepOrange = "#FF5722";

    /// <summary>
    /// The color brown.
    /// </summary>
    public const string Brown = "#795548";

    /// <summary>
    /// The color blue grey.
    /// </summary>
    public const string BlueGrey = "#607D8B";

    /// <summary>
    /// The color grey.
    /// </summary>
    public const string Grey = "#9E9E9E";
}
