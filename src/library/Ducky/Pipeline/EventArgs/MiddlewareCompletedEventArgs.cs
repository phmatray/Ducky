using Ducky.Middlewares;

namespace Ducky.Pipeline;

/// <summary>
/// Event published when a middleware completes processing an action.
/// </summary>
public class MiddlewareCompletedEventArgs : PipelineEventArgs
{
    /// <summary>
    /// Initializes a new instance of the <see cref="MiddlewareCompletedEventArgs"/> class.
    /// </summary>
    /// <param name="context">The action context.</param>
    /// <param name="middleware">The middleware instance.</param>
    /// <param name="phase">The phase, either "Before" or "After".</param>
    public MiddlewareCompletedEventArgs(IActionContext context, IStoreMiddleware middleware, StoreMiddlewarePhase phase)
    {
        Context = context;
        Middleware = middleware;
        Phase = phase;
    }

    /// <summary>
    /// The action context.
    /// </summary>
    public IActionContext Context { get; }

    /// <summary>
    /// The middleware instance.
    /// </summary>
    public IStoreMiddleware Middleware { get; }

    /// <summary>
    /// The phase, either "Before" or "After".
    /// </summary>
    public StoreMiddlewarePhase Phase { get; }
}
